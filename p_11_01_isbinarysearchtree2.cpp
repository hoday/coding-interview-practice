...
template <typename T>
bool isBinarySearchTreeHelper(const std::shared_ptr<Node<T>> &pNode, T lowerBound, T upperBound) {
	if (!pNode) {
		return true;
	} else {
		return 
			(pNode->data >= lowerBound) && 
			(pNode->data <= upperBound) &&
			(isBinarySearchTreeHelper(pNode->pLeft, lowerBound, pNode->data)) &&
			(isBinarySearchTreeHelper(pNode->pRight, pNode->data, upperBound));
	}
}
...
bool isBinarySearchTree(const std::shared_ptr<Node<T>> &pNode) {
	return isBinarySearchTreeHelper(pNode, numeric_limits<T>::min(), numeric_limits<T>::max());
}
